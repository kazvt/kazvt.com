...
function localFairyDustCursor(options = {}) {
  ...
  const shadowBlur = normalizeRange(options.shadowBlur ?? options.blur ?? options.glowBlur, Math.max(0.6, size * 0.08), 0, 48);
  const shadowColor = options.shadowColor || options.shadow || options.glowColor || "rgba(0, 0, 0, 0.24)";
