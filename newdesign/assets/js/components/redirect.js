import { loadSecrets } from "./private-config.js";
...
function getRedirectConfig(secrets) {
...
async function initRedirect() {
  ...
  startProgress(delay);
  document.querySelectorAll("[data-redirect-now]").forEach((element) => {
    element.addEventListener("click", () => redirectTo(target));
  });
  window.setTimeout(() => redirectTo(target), delay * 1000);
}
